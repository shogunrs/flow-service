...
public class MongoStageRepository implements StageRepository {
    ...
    private static Stage toDomain(StageDocument d) {
        ...
        return new Stage(d.id, d.processKey, d.title, d.slaDays, d.color, d.order, d.createdAt, d.updatedAt);
    }
    private static StageDocument toDoc(Stage s) {
        StageDocument d = new StageDocument();
        d.id = s.getId();
        d.processKey = s.getProcessKey();
        d.title = s.getTitle();
    ...
    @Override public List<Stage> findByProcessKeyOrderByOrder(String processKey) {
        return repo.findByProcessKeyOrderByOrderAsc(processKey).stream().map(MongoStageRepository::toDomain).collect(Collectors.toList());
    }
    @Override public void deleteByProcessKey(String processKey) { repo.deleteByProcessKey(processKey); }
